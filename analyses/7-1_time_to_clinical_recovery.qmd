...
mlogit_mod <- cmdstan_model("stan/time_to_event_competing_reduced_alt.stan")
mlogit_linear <- cmdstan_model("stan/time_to_event_competing_reduced_linear.stan")
mlogit_rw <- cmdstan_model("stan/time_to_event_competing_reduced_rw.stan")
mlogit_no_epoch <- cmdstan_model("stan/time_to_event_competing_no_epoch.stan") 
mlogit_no_site <- cmdstan_model("stan/time_to_event_competing_no_site.stan") 
```
...
get_arm_specific_probs <- function(drws) {
  haztbl <- vector("list", nrow(drws$Ccon))
  names(haztbl) <- rownames(drws$Ccon)
  for (k in 1:nrow(drws$Ccon)) {
    haz <- t(do.call(cbind, apply(sweep(drws$alpha, 2, drws$Ccon[k, ], "+"), 1, mlogit)))
...
dat <- acs_itt_outdat %>% 
  select(StudyPatientID, time, out_rec, out_ttr, y, y0, y1, y2, epoch, site_num, ctry_num, agegte60) %>% 
  mutate(id = as.numeric(factor(StudyPatientID))) %>% 
  select(-StudyPatientID) %>%
  left_join(tibble(id = 1:max(.$id), trt = sample.int(4, max(.$id), replace = T)), by = "id")
saveRDS(dat, "dat.rds")
```
...
haz <- get_arm_specific_probs(drws)
pdat <- haz %>% 
  select(intervention, time, unrecovered = surv, recovered = cprob1, died = cprob2) %>%
  pivot_longer(unrecovered:died, names_to = "event", values_to = "posterior")
ggplot(pdat, aes(time, fill = event, colour = event)) +
...
acs_itt_concurc3_dat %>%
  group_by(CAssignment, out_rec) %>%
  summarise(
    n = n(),
    median = median(out_ttr),
    mean = mean(out_ttr)
  )
```
