...
p <- tmp_agg %>% 
  ...
  theme(panel.grid.minor = element_blank())
path <- file.path("outputs", "figures", "outcomes")
ggsave(file.path(path, "7-1-country.pdf"), p, height = 3, width = 6)
p
```
...
p <- tmp_agg %>% 
  ...
  theme(panel.grid.minor = element_blank())
path <- file.path("outputs", "figures", "outcomes")
ggsave(file.path(path, "7-1-age.pdf"), p, height = 3, width = 6)
p
```
...
mod <- cmdstan_model("stan/time_to_event_competing_reduced_alt.stan")
ctr <- contr.orthonorm
...
as_tibble(drws$alpha, rownames = "day") %>% 
  ggplot(., 
         aes(xdist=V2, y = fct_inorder(day))) + 
  stat_slabinterval()
```
