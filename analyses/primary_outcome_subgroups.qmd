...
The pre-specified subgroup analysis was for patients who were "on room air in the preceding 24 hours **or** had an oxygen saturation < 94" vs "not on room air and oxygen saturation $\geq$ 94. However, oxygen saturation was only reported for participants who were reported as on room air in the preceding 24 hours. For participants who were not on room air, their oxygen saturation was unknown. Therefore, the pre-specified subgroup is not identifiable.
...
Instead, what has been analysed below is the subgroup "not on room air **or** on room air with oxygen saturation > 94" vs "on room air and oxygen saturation < 94".
...
# Derive subgroup-specific effects
labs <- c("No room air or SpO2 \u2265 94", "Room air and SpO2 < 94")
betaC <- res$drws$beta[grepl("CAssignment", names(res$drws$beta))]
res$drws$trt1 <- betaC[1:3]
res$drws$trt2 <- betaC[1:3] + betaC[4:6]
res$drws$logOR <- rbind(res$drws$trt1, res$drws$trt2)
