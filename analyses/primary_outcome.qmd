...
ors <- mdrws$OR[9:length(mdrws$OR)]
orsdat <- tibble(
  Group = "Epoch",
...
tibble(
  ...
  RV = c(mdrws$OR[1:3], mdrws$OR[1] / mdrws$OR[3])) %>% 
  ggplot(., aes(y = Contrast, xdist = RV)) + 
  stat_halfeye(
    aes(fill = 
          stat(cut_cdf_qi(
...
tibble(
    ...
    `Pr(OR < 1)` = Pr(mdrws$OR[1:3] < 1),
    Median = median(mdrws$OR[1:3]),
    Mean = E(mdrws$OR[1:3]),
    `95% CrI` = apply(
      quantile(mdrws$OR[1:3], c(0.025, 0.975)), 2, 
      function(x) sprintf("(%.2f, %.2f)", x[1], x[2]))
  ) %>%
...
subgrp_effects <- expand_grid(
  Country = c("India", "Australia", "Nepal", "New Zealand"),
  Intervention = c("Intermediate", "Standard plus aspirin", "Therapeutic"))
subgrp_effects$posterior <- exp(c(mdrws$beta_trt1, mdrws$beta_trt2, mdrws$beta_trt3, mdrws$beta_trt4))
```

```{r}
ggplot(subgrp_effects, aes(xdist = posterior, y = Country)) +
