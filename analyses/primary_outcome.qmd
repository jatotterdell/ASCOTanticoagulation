...
plot_grp_ppc <- function(dat, lab = "") {
  dat %>%
    ...
    stat_histinterval(fatten_point = 1) +
    geom_segment(aes(x = mean_y, 
                     xend = mean_y,
                     y = 1:nrow(dat),
                     yend = 2:(nrow(dat)+1)-0.1), colour = "red") +
    labs(
...
p4 <- plot_grp_ppc(ppc_site %>% filter(Country == "IN"), "Sites India")  + labs(x = "")
p5 <- plot_grp_ppc(ppc_site %>% filter(Country == "AU"), "Sites Australia") + labs(x = "")
p6 <- plot_grp_ppc(ppc_site %>% filter(Country == "NP"), "Sites Nepal")
p7 <- plot_grp_ppc(ppc_site %>% filter(Country == "NZ"), "Sites NZ")
```

```{r}
#| label: fig-primary-model-acs-itt-ppc-1
p <- ((p3 | p1 / p2) / ( (p4 | p5) / (p6 | p7) + plot_layout(heights = c(2, 1)) ) ) +
  plot_layout(heights = c(1, 1.5))
p
pth <- file.path("outputs", "figures", "outcomes", "primary",
                 "primary-model-acs-itt-ppc.pdf")
ggsave(pth, p, width = 6, height = 5)
```
