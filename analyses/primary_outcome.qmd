...
fit_primary_model <- function(
  ...
  # Get other comparison odds ratios
  mdrws$ORother <- exp(c(
    "Intermediate vs. low with aspirin" = mdrws$Ctrt[1] - mdrws$Ctrt[2],
    "Intermediate vs. therapeutic" = mdrws$Ctrt[1] - mdrws$Ctrt[3],
    "Low with aspirin vs. therapeutic" = mdrws$Ctrt[2] - mdrws$Ctrt[3]))
...
```{r}
#| label: tbl-other-contrasts
#| tbl-cap: Other intervention contrasts
save_tex_table(
  odds_ratio_summary_table(res$drws$ORother, "latex"),
  "outcomes/primary/primary-model-acs-itt-summary-table-other-contrasts")
odds_ratio_summary_table(res$drws$ORother)
```
...
Cc <- attr(X, "contrasts")$CAssignment  
sdrws$Ccon <- as.vector(Cc %**% sdrws$beta[grepl("CAssignment[0-9]", names(sdrws$beta))])
names(sdrws$Ccon) <- rownames(Cc)
sdrws$Ctrt <- sdrws$Ccon[-1] - sdrws$Ccon[1]

sdrws$beta_trt <- as.vector(
  transform_coding(cbind(1, ctr(2)), cbind(1, contr.treatment(2))) %**% sdrws$beta[1:2]
)
sdrws$OR <- c(
  "Intermediate-dose" = exp(sdrws$Ctrt),
...
sdrws$OR <- c(
  "Intermediate-dose" = exp(sdrws$Ctrt[1]),
  "Low-dose with aspirin" = exp(sdrws$Ctrt[2]),
...
# Get other comparison odds ratios
sdrws$ORother <- exp(c(
  "Intermediate vs. low with aspirin" = sdrws$Ctrt[1] - sdrws$Ctrt[2]
))
```
...
```{r}
#| label: tbl-acs-itt-c3-primary-model-summary-other-contrasts
#| tbl-cap: Parameter posterior summaries.
OR <- sdrws$ORother
tdat <- tibble(
    Parameter = names(OR),
    Median = median(OR),
    `95% CrI` = sprintf("(%.2f, %.2f)", quantile(OR, 0.025), quantile(OR, 0.975)), 
    `Mean (SD)` = sprintf("%.2f (%.2f)", E(OR), sd(OR)),
    `Pr(OR < 1)` = Pr(OR < 1),
)
...
save_tex_table(
  ...
  kable_styling(
    ...
    bootstrap_options = "striped", 
    latex_options = "HOLD_position"),
  "outcomes/primary/primary-model-acs-itt-c3-summary-table-other-contrasts")
...
# Get other comparison odds ratios
sdrws$ORother <- exp(c(
  "Intermediate vs. therapeutic" = sdrws$Ctrt[1] - sdrws$Ctrt[2]
))
```
...
```{r}
#| label: tbl-acs-itt-c4-primary-model-summary-other-contrasts
#| tbl-cap: Parameter posterior summaries.
OR <- sdrws$ORother
tdat <- tibble(
    Parameter = names(OR),
...
save_tex_table(
  ...
  kable_styling(
    ...
    bootstrap_options = "striped", 
    latex_options = "HOLD_position"),
  "outcomes/primary/primary-model-acs-itt-c4-summary-table-other-contrasts")
...
### Sensitivity: Age as linear term

```{r}
res <- fit_primary_model(
  acs_itt_nona_dat,
  primary_mod,
  vars = c("inelgc3", "age_c", "ctry"),
  beta_sd_var = c(10, 2.5, 1, 1),
  beta_sd_trt = 10,
  seed = 32915,
  adapt_delta = 0.99
...
names(res$drws$OR) <- c(
  intervention_labels2()$CAssignment[-(1:2)], 
  "Ineligible aspirin", "Age", "Australia/New Zealand", "Nepal")
```
