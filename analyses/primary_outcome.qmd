...
mort_table <- all_dat %>%
  ...
  summarise(
    ...
    `Mortality_Unknown at day 28_` = sum(is.na(D28_death))
  ) %>%
  gather(key, Frequency) %>%
    separate(key, into = c("Measure", "Outcome", "Breakdown"), sep = "_") %>%
    mutate(`Percentage` = sprintf("%2.1f", 100 * Frequency / nrow(fas_itt_dat)))
kable(
  mort_table %>% select(-1),
...
vaso_table <- all_dat %>%
  ...
  summarise(
    ...
    `Vasopressor/inotropes_Unknown_Total` = sum(is.na(ANY_vasop)),
    `Vasopressor/inotropes_Unknown_Missing day 28` = sum(is.na(D28_vasop)),
    `Vasopressor/inotropes_Unknown_Missing daily records` = 
      sum(is.na(DAILY_missing) | DAILY_missing == 1),
  ) %>%
  gather(key, Frequency) %>%
    separate(key, into = c("Measure", "Outcome", "Breakdown"), sep = "_") %>%
    mutate(`Percentage` = sprintf("%2.1f", 100 * Frequency / nrow(fas_itt_dat)))
...
vent_table <- all_dat %>%
  ...
  summarise(
    ...
    `Ventilation_Unknown_Total` = sum(is.na(ANY_vent)),
    `Ventilation_Unknown_Missing daily records` = sum(is.na(DAILY_missing) | DAILY_missing == 1),
      `Ventilation_Unknown_Unknown status at day 28` = 
      sum(ANY_DD_vent == 0 & is.na(D28_vent), na.rm = TRUE),
  ) %>%
  gather(key, Frequency) %>%
    separate(key, into = c("Measure", "Outcome", "Breakdown"), sep = "_") %>%
    mutate(`Percentage` = sprintf("%2.1f", 100 * Frequency / nrow(fas_itt_dat)))
...
comp_table <- all_dat %>%
  filter_fas_itt()  %>%
  summarise(
    `Total_Total_` = length(PO),
    `Primary Outcome_Yes_` = sum(PO, na.rm = TRUE),
    `Primary Outcome_No_`  = sum(1 - PO, na.rm = TRUE),
    `Primary Outcome_Unknown_Total` = sum(is.na(PO)),
    `Primary Outcome_Unknown_Unknown day 28 status` = sum(is.na(PO) & is.na(D28_death)),
    `Primary Outcome_Unknown_Unknown vasopressor/inotrope use (known day 28 status)` = 
      sum(is.na(PO) & !is.na(D28_death) & is.na(ANY_vasop)),
    `Primary Outcome_Unknown_Missing any daily records` = 
      sum((is.na(PO) & is.na(DAILY_missing)) | (!is.na(DAILY_missing) & DAILY_missing == 1))
  ) %>%
  gather(key, Frequency) %>%
    separate(key, into = c("Measure", "Outcome", "Breakdown"), sep = "_") %>%
    mutate(`Percentage` = sprintf("%2.1f", 100 * Frequency / nrow(fas_itt_dat)))
...
kable(
  ...
  kable_styling(font_size = 11) %>%
  collapse_rows(1, valign = "top", latex_hline = 'custom', custom_latex_hline = 1) %>%
  group_rows("Primary outcome", 1, 8) %>%
  group_rows("Mortality", 8, 12) %>%
  group_rows("Vasopressor/inotropes", 13, 19) %>%
  group_rows("Ventilation", 20, 26)
```
...
levs <- c("None", "Death + V/I + Vent", "Death + V/I", "Death + Vent", "V/I + Vent", 
          "Death", "V/I", "Vent", "Unknown")
all_dat %>%
  filter_fas_itt()  %>%
  summarise(
    `Composite outcomes` = factor(case_when(
      PO == 0 ~ 0,
      is.na(PO) ~ 99,
  ...
  dplyr::count(`Composite outcomes`) %>%
  kable() %>%
  kable_styling()
```
...
```{r}
#| label: load-trtonly-acs-itt-model
...
for(i in 1:4) {
  mod_dat <- make_model_data_trt_only(acs_itt_nona_dat %>% filter(ctry == ctry_vals[i]))
  seed <- 713509
  snk <- capture.output(
...
seed <- 71236
mod_dat <- make_model_data_noepochsite(acs_itt_dat)
snk <- capture.output(
...
#| code-summary: Data and sampling no site or epoch model
seed <- 71236
mod_dat <- make_model_data_noepoch(acs_itt_dat)
snk <- capture.output(
...
seed <- 32915
mod_dat <- make_model_data_primary(acs_itt_dat)
snk <- capture.output(
