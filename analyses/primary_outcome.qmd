...
make_model_data_trt_only <- function(dat) {
  ...
  N <- dim(X)[1]
  K <- dim(X)[2]  
  list(N = N, K = K, X = X, y = y, beta_sd = c(2.5, 1, 1, 1, 1, 1))
}
...
make_model_data_noepochsite <- function(dat) {
  ...
  list(N = N, K = K, X = X, y = y, 
       beta_sd = c(2.5, 1, 1, 1, 1, 1, 2.5, 1, 1, 1, 10))
}
...
make_model_data_noepoch <- function(dat) {
  ...
  list(N = N, K = K, X = X, y = y,
       ...
       beta_sd = c(2.5, 1, 1, 1, 1, 1, 2.5, 10))
}
...
mod_dat <- make_model_data_trt_only(acs_itt_dat)
snk <- capture.output(
...
```{r}
#| label: tbl-posterior-summaries-trt-only
...
seed <- 32915
primary_mod <- cmdstan_model(
  "stan/primary_logistic_model_site_epoch.stan")
...
```{r}
#| code-summary: Setup ACS-ITT dataset
fas_itt_dat <- all_dat %>%
    filter(
      ENR_rec == 1, 
      WTH_FU == 0,
      !is.na(PO))
site_counts <- fas_itt_dat %>% 
  dplyr::count(
    Country = factor(Country, levels = c("IN", "AU", "NP", "NZ")), 
    Location)
merge_aus <- site_counts %>% 
...
```{r}
mod_dat <- make_model_data_primary(fas_itt_dat)
...
```{r}
#| label: primary-posteriors-fas
#| code-summary: Posterior draws FAS
Ca <- cbind(1, attr(mod_dat$X, "contrasts")$AAssignment)
Cc <- cbind(1, attr(mod_dat$X, "contrasts")$CAssignment)
modrv <- as_draws_rvars(
  primary_fit$draws(
    c("beta", "beta_region", "gamma_site", "tau_site", "gamma_epoch", "tau_epoch")))
modrv$eta <- as.vector(Cc %**% modrv$beta[1:5])
modrv$beta_uncon <- as.vector(Cc[, -1] %**% modrv$beta[2:5])
modrv$prob <- 1 / (1 + exp(-modrv$eta))
modrv$beta_trt <- as.vector(
  transform_coding(Cc, cbind(1, contr.treatment(5))) %**% 
  modrv$beta[1:5])
modrv$OR <- setNames(exp(modrv$beta_trt[-(1:2)]), c("C2", "C3", "C4"))
```
...
```{r}
#| label: tbl-posterior-summaries-primary-fas
#| code-summary: Posterior summaries FAS
#| tbl-cap: Posterior summaries FAS
tibble(
    Parameter = c("OR[C2]", "OR[C3]", "OR[C4]"),
    `Pr(OR < 1)` = Pr(modrv$OR < 1),
    Median = median(modrv$OR),
