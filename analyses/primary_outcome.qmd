...
mdat <- make_model_data_primary(acs_itt_nona_dat)
snk <- capture.output(
...
mdat <- make_stan_data(
  ...
  "PO",
  c(2.5, rep(1, 5), 2.5, 10, rep(2.5, 3), rep(1, 9)))
snk <- capture.output(
...
subgrp_effects <- expand_grid(
  Country = c("India", "Australia", "Nepal", "New Zealand", "Primary Model"),
  Intervention = c("Intermediate", "Standard plus aspirin", "Therapeutic"))
subgrp_effects$posterior <- exp(
  c(mdrws$beta_trt1, mdrws$beta_trt2, mdrws$beta_trt3, mdrws$beta_trt4, log(prim_mdrws$OR[1:3])))
```

```{r}
p <- ggplot(subgrp_effects, aes(xdist = posterior, y = Country)) +
...
subgrp_effects <- expand_grid(
  ...
  Intervention = c("Intermediate", "Standard plus aspirin", "Therapeutic"))
subgrp_effects$posterior <- exp(c(mdrws$beta_trt1, mdrws$beta_trt2, log(prim_mdrws$OR[1:3])))
```

```{r}
p <- ggplot(subgrp_effects, aes(xdist = posterior, y = Age)) +
  facet_grid(Intervention ~ .) +
  stat_pointinterval() +
  geom_vline(xintercept = 1, linetype = 2) +
  scale_x_log10("Odds ratio (log scale)") +
...
```{r}
p <- ggplot(subgrp_effects, aes(xdist = posterior, y = `Days since first symptom`)) +
  facet_grid(Intervention ~ .) +
  stat_pointinterval() +
  geom_vline(xintercept = 1, linetype = 2) +
  scale_x_log10("Odds ratio (log scale)") +
