...
description: |
  *This scripot is used to undertake the analysis of the primary outcome for ASCOT.
  In the hope it helps with readability and making each standalone analysis clearer, 
  verbosity has been preferred to concisenes with respect to the R code used for the analyses.*
...
mdat <- make_stan_data(
  ...
  "PO",
  beta_sd = c(2.5, 1, 1, 1, 1, 1, 10, 2.5),
  contr.orthonorm
)
...
prim_mdrws$OR <- c(
  prim_mdrws$OR, 
  "Ineligible aspirin (ref eligible)" = exp(prim_mdrws$beta[which(grepl("inelg", colnames(mdat$X)))]),
  "Age 60+ (ref < 60)" = exp(prim_mdrws$beta[which(grepl("age", colnames(mdat$X)))]),
  setNames(exp(prim_mdrws$beta_region), c("India (ref)", "Australia/New Zealand", "Nepal")),
...
odds_ratio_summary_table(prim_mdrws$OR[1:8])
```
...
epoch_ors <- exp(prim_mdrws$gamma_epoch)
names(epoch_ors) <- epoch_map$epoch_lab

orsdat <- tibble(
  Group = "Epoch",
  Parameter = fct_inorder(names(epoch_ors)),
  posterior = epoch_ors
)
...
site_ors <- exp(prim_mdrws$gamma_site)
names(site_ors) <- site_map$site
orsdat <- tibble(
  Group = "Site",
  Country = factor(mdat$region_by_site, labels = c("India", "Australia\nNew Zealand", "Nepal")),
  Parameter = fct_inorder(names(site_ors)),
  posterior = site_ors
)
...
mdrws$OR <- setNames(
  ...
  mdrws$OR <- c(
    ...
    setNames(exp(mdrws$beta_region), c("India (ref)", "Australia/New Zealand", "Nepal")),
...
```{r}
#| label: odds-ratio-summary-primary-model-epoch-site-terms
epoch_ors <- exp(mdrws$gamma_epoch)
names(epoch_ors) <- epoch_map$epoch_lab
...
X <- model.matrix(
  ...
  data = acs_itt_nona_c2_dat,
  contrasts.arg = list(CAssignment = ctr))
y <- acs_itt_nona_c2_dat$PO
sdat <- list(
  N = nrow(X),
  K = ncol(X),
  X = X,
...
```{r}
#| label: odds-ratio-summary-table-primary-model-acs-itt-c2-save
save_tex_table(
...
```{r}
acs_itt_c3_dat <- acs_itt_dat2 %>%
  filter(RandDate < get_intervention_dates()$endate[3]) %>%
  mutate(
    CAssignment = droplevels(CAssignment),
    ctry = droplevels(ctry)
  )
acs_itt_nona_c3_dat <- acs_itt_c3_dat %>% 
  filter(!is.na(PO))
```

```{r}
#| label: tbl-acs-itt-c3-summary-table
make_po_table(acs_itt_c3_dat)
```
...
X <- model.matrix(
  ~ CAssignment + inelgc3 + agegte60 + ctry, 
  data = acs_itt_nona_c3_dat,
  contrasts.arg = list(CAssignment = contr.orthonorm))
y <- acs_itt_nona_c3_dat$PO
sdat <- list(
  N = nrow(X),
  K = ncol(X),
  X = X,
  y = y,
  beta_sd = c(2.5, 1, 1, 10, 2.5, 1)
)
snk <- capture.output(
  sfit <- logistic_mod$sample(
    data = sdat,
...
sdrws$OR <- c(
  ...
  "Ineligible aspirin" = exp(sdrws$beta[4]),
  "Age 60+" = exp(sdrws$beta[5]),
  "Aus/NZ" = exp(sdrws$beta[6])
)
```
...
```{r}
#| label: odds-ratio-summary-table-primary-model-acs-itt-c3-save
save_tex_table(
  odds_ratio_summary_table(sdrws$OR, "latex"),
...
X <- model.matrix(
  ~ CAssignment + agegte60 + ctry,
  # ~ CAssignment, 
  data = acs_itt_nona_c4_dat,
  contrasts.arg = list(CAssignment = contr.orthonorm))
y <- acs_itt_nona_c4_dat$PO
sdat <- list(
  N = nrow(X),
  K = ncol(X),
  X = X,
...
```{r}
#| label: odds-ratio-summary-table-primary-model-acs-itt-c4-save
save_tex_table(
  odds_ratio_summary_table(sdrws$OR, "latex"),
...
In the following sensitivity analysis, the hiearchical site and epoch terms are removed from the primary model.
