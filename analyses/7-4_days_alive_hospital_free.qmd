...
ordmod_epoch <- cmdstan_model("stan/logistic_cumulative_epoch.stan")
ordmod <- cmdstan_model("stan/logistic_cumulative_epoch_site.stan")
```
...
make_model_data <- function(
  ...
  if (is.null(vars)) {
    xform <- as.formula(" ~ CAssignment + AAssignment")
  } else {
    xform <- as.formula(paste(" ~ CAssignment + AAssignment + ", paste(vars, collapse = " + ")))
  }
  X <- model.matrix(
    xform,
    data = dat %>% mutate(AAssignment = relevel(AAssignment, "A0")),
    contrast = list(
      AAssignment = ctr,
      CAssignment = ctr
    )
  ...
  attr(X, "contrasts")$CAssignment = ctr(nCXassign + 1)
  attr(X, "contrasts")$AAssignment = ctr(nAXassign + 1)
  nXassign <- sum(grepl("Assignment", colnames(X)))
  
  beta_sd <- c(rep(1, nXassign), beta_sd_var)
...
add_treatment_rvs <- function(rvs, mdat) {
  ...
  rvs$OR <- setNames(exp(rvs$beta_trt), c("Intermediate", "Standard plus aspirin", "Therapeutic"))
  return(rvs)
}
```
