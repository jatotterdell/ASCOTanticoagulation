...
tdat <- acs_itt_concurc2_dat  %>%
  group_by(CAssignment = factor(CAssignment, levels = c("C1", "C2"), labels = c("Standard", "Intermediate"))) %>%
  summarise(
    Patients = n(),
    Known = sum(!is.na(out_dafh)),
    Deaths = sprintf("%i (%.0f%%)", sum(D28_death, na.rm = TRUE), 100 * mean(D28_death, na.rm = TRUE)),
...
save_tex_table(tab, file.path("outcomes", "secondary", "7-4-anticoagulation-concurrent-intermediate-summary"))
...
```{r}
#| code-summary: Descriptive summary table
tdat <- acs_itt_concurc3_dat %>%
  group_by(CAssignment = factor(CAssignment, levels = c("C1", "C2", "C3"), labels = c("Standard", "Intermediate", "Standard plus aspirin"))) %>%
  summarise(
    Patients = n(),
    Known = sum(!is.na(out_dafh)),
    Deaths = sprintf("%i (%.0f%%)", sum(D28_death, na.rm = TRUE), 100 * mean(D28_death, na.rm = TRUE)),
...
tab <- kable(
  ...
  row_spec(nrow(tdat), bold = T)
save_tex_table(tab, file.path("outcomes", "secondary", "7-4-anticoagulation-concurrent-stdaspirin-summary"))
...
```{r}
#| code-summary: Descriptive summary table
tdat <- acs_itt_concurc4_dat %>%
  group_by(CAssignment = factor(CAssignment, levels = c("C1", "C2", "C4"), labels = c("Standard", "Intermediate", "Therapeutic"))) %>%
  summarise(
    Patients = n(),
    Known = sum(!is.na(out_dafh)),
    Deaths = sprintf("%i (%.0f%%)", sum(D28_death, na.rm = TRUE), 100 * mean(D28_death, na.rm = TRUE)),
...
tab <- kable(
  ...
  row_spec(nrow(tdat), bold = T)
save_tex_table(tab, file.path("outcomes", "secondary", "7-4-anticoagulation-concurrent-therapeutic-summary"))
