...
fit_primary_model <- function(
  ...
  if(fit) {
    ...
    if (save) {
      mfit$save_object(file.path("outputs", "models", "secondary", "7-4-dafh", "primary.rds"))  
    }
  } else {
    mfit <- readRDS(file.path("outputs", "models", "secondary", "7-4-dafh", "primary.rds"))
  }
...
res <- fit_primary_model(acs_itt_nona_dat, ctr = contr.treatment, fit = TRUE, save = FALSE)
```
...
-   **Set**: patients randomised to either low-dose or intermediate-dose whilst anticoagulation domain remained open.
...
res <- fit_acs_itt_intermediate(FALSE)
mdat <- res$dat
mfit <- res$fit
mdrws <- res$drws
```
