...
fas_itt_dat <- all_dat %>% 
  ...
  left_join(all_dat %>% select(StudyPatientID, D28_BreathScale, D28_death), by = "StudyPatientID") %>%
...
descriptive_table <- acs_itt_dat %>% 
  mutate(D28_BreathScale = fct_explicit_na(D28_BreathScale)) %>%
  dplyr::count(CAssignment, D28_BreathScale) %>%
...
acs_itt_nona_dat %>%
  dplyr::count(CAssignment, D28_BreathScale = D28_BreathScale) %>%
  group_by(CAssignment) %>%
  mutate(p = n / sum(n)) %>%
  ggplot(., aes(CAssignment, p)) +
  geom_col(aes(fill = D28_BreathScale)) + 
  scale_fill_viridis_d(option = "B", name = "mMRC breathlessness scale (day 28)") + 
