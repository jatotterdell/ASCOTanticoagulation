...
who_labels_short <-c("No hospital, no limitations",
                     "No hospital, limitations",
               "Hospital, no ongoing care ",
               "Hospital, requiring ongoing care",
               "Hospital, requiring oxygen",
               "Hospital, non-invasive ventilation ",
               "Hospital, invasive ventilation/ECMO",
               "Death")
...
check_ests <- function(region_ind) 
{
  ...
  p_check <- acs_itt_nona_dat %>%
    ...
    dplyr::count(site, D28_who, .drop = F) %>%
    group_by(site) %>%
    mutate(p = n / sum(n)) %>%
    ungroup() %>%
  ...
  ggplot(p_check, aes(reorder(D28_who, desc(D28_who)), p)) +
    facet_wrap( ~ site) +
    ...
    ylab("Proportion in category") + xlab("WHO disease severity score (day 28)")
...
{
  y_ppc <- as_draws_df(stan_full[["draws"]]("y_ppc"))[,-c(1531:1533)]
  ...
  f_1 <- function(x)
  {
    ...
    gp <- gp %>% mutate(cat = factor(cat, levels = 1:8, labels = who_labels))
    gp <- left_join(key, gp, by = c("group", "cat")) %>% select(freq)
    colnames(gp) <- NULL
    gp
  }
  
  gp_dat <- lapply(1:100, f_1)
...
plot_ordinal_ppd(variable = "D28_who", grouper = "CAssignment")
plot_ordinal_ppd(variable = "D28_who", grouper = "AAssignment")
plot_ordinal_ppd(variable = "D28_who", grouper = "country")
plot_ordinal_ppd(variable = "D28_who", grouper = "site")
plot_ordinal_ppd(variable = "D28_who", grouper = "epoch")
...
rvs_site   <- as_draws_rvars(stan_full[["draws"]](c("xi_site")))[[1]]
rvs_epoch  <- as_draws_rvars(stan_full[["draws"]](c("tau_epoch")))[[1]]
...
names(site_lab) <- c("number", "label", "site")
site_lab   <- site_lab %>% mutate(label = paste0(label, ": ", site)) 
...
make_outcome_table <- function(OR, format = "html", fn = NULL)
{
  out <- tibble(Parameter = names(OR),
    ...
    group_rows("Site", 9, 35) %>%
    group_rows("Epoch", 36, 47) %>%
    add_footnote("Note: Epoch is the 4-week interval relative to the most recently randomised participant.", notation = "none", escape = TRUE)
...
make_outcome_table(rvs_all, format = "latex", fn = "7-2_who_ordinal_scale")
