...
seed <- 59579
model_full <- cmdstan_model(file.path("stan", "ordinal_site_epoch.stan"))
```
...
make_stan_data <- function(dat, vars, outcome, beta_sd = c(2.5, rep(1, nXassign), 2.5, 10)) 
{
  ...
  Ca <- cbind(1, attr(X, "contrasts")$AAssignment) 
  Cc <- cbind(1, attr(X, "contrasts")$CAssignment)
...
dat_full[["y"]] <- as.integer(dat_full[["y"]])
...
stan_full$summary(variables = c("p0",
                                "alpha",
                                "beta", 
                                "beta_region", 
...
mcmc_trace(stan_full$draws("p0",            format = "matrix"))
...
rvs_p      <- as_draws_rvars(stan_full$draws(c("p0")))
