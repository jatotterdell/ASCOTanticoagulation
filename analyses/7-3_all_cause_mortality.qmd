...
```{r}
with(acs_itt_nona_dat, table(D28_death, CAssignment))
```
...
make_stan_data <- function(dat, vars, outcome, beta_sd = c(2.5, rep(1, nXassign), 2.5, 10), ctr = contr.orthonorm) 
{
  X <- model.matrix(
...
dat_full <- make_stan_data(dat = acs_itt_nona_dat, 
                           vars    = c("CAssignment", "AAssignment", "agegte60", "inelgc3"),
                           outcome = "D28_death")
```
...
make_outcome_table <- function(OR, format = "html", fn = NULL)
{
  out <- tibble(Parameter = names(OR),
    ...
    footnote(none = "Note: Epoch is the 4-week interval relative to the most recently randomised participant.",
             escape = TRUE)
...
save_tex_table(
  odds_ratio_summary_table(rvs_all[1:10], "latex"),
...
orsdat <- tibble(
  Group = "Site",
  Country = factor(dat_full$region_by_site, labels = c("India", "Australia", "Nepal", "New\nZealand")),
