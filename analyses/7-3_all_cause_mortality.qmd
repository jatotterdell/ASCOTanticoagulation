...
rvs_region <- setNames(exp(rvs_region), c("India", "Australia/New Zealand", "Nepal"))
...
rvs_all <- c(rvs_trt, rvs_inlgc3, rvs_agegrp, rvs_region[-1])
```
...
p <- plot_epoch_site_terms(
  rvs_epoch, 
  rvs_site, 
  factor(dat_full$region_by_site, labels = c("India", "Australia\nNew Zealand", "Nepal")))
pth <- file.path(
...
names(mdrws$beta) <- colnames(mdat$X)
mdrws$Ccon <- attr(dat_full$X, "contrasts")$randC %**% mdrws$beta[2:4]
mdrws$Ctrt <- mdrws$Ccon[-1] - mdrws$Ccon[1]
mdrws$OR <- c(
  setNames(exp(mdrws$Ctrt), c("Intermediate", "Standard plus aspirin", "Therapeutic")),
  setNames(exp(mdrws$beta[-(1:6)]), c("Ineligible aspirin", "Age 60+", "Australia/New Zealand", "Nepal"))
