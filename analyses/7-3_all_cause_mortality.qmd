...
model_full <- cmdstan_model(file.path("stan", "primary_logistic_model_site_epoch2.stan"))
model_site_only <- cmdstan_model(file.path("stan", "primary_logistic_model_site.stan"))
model_epoch_only <- cmdstan_model(file.path("stan", "primary_logistic_model_epoch.stan")) 
model_fixed_only <- cmdstan_model(file.path("stan", "logistic_bernoulli.stan"))
```
...
mdrws$compare <- c(
  "Intermediate vs standard" = exp(mdrws$Ctrt[1]),
  "Standard plus aspirin vs standard" = exp(mdrws$Ctrt[2]),
  "Therapeutic vs standard" = exp(mdrws$Ctrt[3]),
  ...
  "Standard plus aspirin vs therapeutic" = exp(mdrws$Ctrt[2] - mdrws$Ctrt[3])
)
mdrws$OR <- c(
  setNames(exp(mdrws$Ctrt), c("Intermediate", "Standard plus aspirin", "Therapeutic")),
...
mdrws <- as_draws_rvars(mfit$draws())
names(mdrws$beta) <- colnames(mdat$X)
names(mdrws$gamma_site) <- acs_itt_concurc2_nona_dat %>% dplyr::count(site_num, site) %>% pull(site)
site_facet <- factor(mdat$region_by_site, labels = c("India", "Australia\nNew Zealand", "Nepal"))
...
mdrws$OR <- c(
  setNames(exp(mdrws$Ctrt), c("Intermediate")),
  setNames(exp(mdrws$beta[-(1:4)]), c("Age 60+", "Australia/New Zealand", "Nepal"))
)
```
...
mdrws$compare <- c("Intermediate vs standard" = exp(mdrws$Ctrt[1]),
                   ...
                   "Intermediate vs standard plus aspirin" = exp(mdrws$Ctrt[1] - mdrws$Ctrt[2]))
mdrws$OR <- c(
  setNames(exp(mdrws$Ctrt), c("Intermediate", "Standard plus aspirin")),
